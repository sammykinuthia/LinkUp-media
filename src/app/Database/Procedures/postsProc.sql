...
CREATE OR ALTER PROC uspGetPosts (@user_id VARCHAR(200)) AS
BEGIN
    SELECT TOP 100
        p.id,
        p.[image],
    ...
    LEFT JOIN (SELECT post_id, COUNT(*) AS likes FROM likePost GROUP BY post_id) lpcount ON lpcount.post_id = p.id
    LEFT JOIN (SELECT user_id, followed_user_id FROM follow WHERE followed_user_id = @user_id) f ON f.user_id = p.user_id
    LEFT JOIN (SELECT post_id, 1 AS liked FROM likePost WHERE user_id = @user_id ) lp ON lp.post_id = p.id
...
CREATE OR ALTER PROC uspGetPostsExplore (@user_id VARCHAR(200)) AS
BEGIN
    SELECT TOP 100
        p.id,
        p.[image],
    ...
    LEFT JOIN (SELECT post_id, COUNT(*) AS likes FROM likePost GROUP BY post_id) lpcount ON lpcount.post_id = p.id
    LEFT JOIN (SELECT post_id, 1 AS liked FROM likePost WHERE user_id = @user_id ) lp ON lp.post_id = p.id
    LEFT JOIN(SELECT user_id, followed_user_id FROM follow WHERE followed_user_id = @user_id) f ON f.user_id = u.id
...
go
    ...
    BEGIN
        SELECT c.id, c.[image], c.[level], c.parent_comment, c.post_id, c.user_id, likes, username, c.[message] FROM comments c
...
CREATE OR ALTER PROC uspLikePost(@user_id VARCHAR(200), @post_id VARCHAR(200)) AS
BEGIN
...
CREATE OR ALTER PROC uspLikeComment(@user_id VARCHAR(200), @comment_id VARCHAR(200)) AS
BEGIN
    INSERT INTO likeComment(user_id, comment_id)
